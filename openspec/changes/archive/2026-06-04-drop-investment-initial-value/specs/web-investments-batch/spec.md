...
- **WHEN** uma linha tem `name`/`currency` vazios
...
A tela SHALL enviar via `POST /api/investments/batch` um **array** de itens (cada um com
`name`, `type`, `currency` e `accountId?`, **sem `initialValue`**) e tratar a resposta:
