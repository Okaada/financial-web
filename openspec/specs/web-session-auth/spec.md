...
(`whoami`): login por navegação top-level para `GET /api/auth/login`, estado de
autenticação inferido por sondagem de endpoint protegido (`401` ⇒ sem sessão) e logout
