...
Definir a tela de **Conta & LGPD** (CONTRACT §9): registrar consentimento
(`POST /account/consent`), exibir a trilha de auditoria da própria conta
(`GET /account/audit`, somente-leitura) e executar a exclusão de conta
(`DELETE /account`, self-only, hard delete irreversível, limpa `fa_session`). Tudo via o
cliente HTTP único, com estados de UI explícitos por operação, confirmação forte na
exclusão e higiene de XSS na auditoria (render estruturado, nunca HTML).
