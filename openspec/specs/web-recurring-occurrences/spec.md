# web-recurring-occurrences

## Purpose

Definir a tela de Previstos do front: listar ocorrências calculadas pelo backend
(`GET /api/recurring-occurrences`, `from`/`to` obrigatórios, não persistidas) e confirmar
uma competência de forma idempotente (`POST /api/recurring-templates/:id/confirm`),
refletindo `confirmed`/`transactionId`, com estados de UI explícitos.
