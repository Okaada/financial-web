...
Definir a camada única de acesso HTTP do front à Finance API: um wrapper de `fetch`
mesmo-origin que centraliza `credentials`, o tratamento de `401` (→ login) e a
interpretação do envelope de erro padrão `{ error: { code, message } }`.
