...
Definir a área de faturas: listar as faturas de um cartão, detalhar uma fatura (com suas
transações) e avançar seu estado pela máquina `open → closed → paid` (fechar/pagar). A UI
apenas reflete o ciclo de vida — não cria nem reabre faturas (isso é responsabilidade do
backend, que as cria preguiçosamente e as reabre ao mudar lançamentos do período).
